...
const IssueDate = ({date, size ="big"}) => {
	...
	let day = new Date(Date.UTC(separateNumberDate[0], separateNumberDate[1], separateNumberDate[2]));
	let options = { weekday: 'short', day: 'numeric', month: 'short' };
	let formatedDay = day.toLocaleDateString('fr', options);
	let capitalizeDay = formatedDay.charAt(0).toUpperCase() + formatedDay.slice(1)
